...
class FabCircularMenuState extends State<FabCircularMenu> with SingleTickerProviderStateMixin {
    ...
    return Container(
      ...
      child: Stack(
        ...
        children: <Widget>[
          // Ring
          Transform(
            transform: Matrix4.translationValues(_translationX, _translationY, 0.0)..scale(_scaleAnimation.value),
                      ...
                      ? Transform.rotate(
                          ...
                          child: Container(
                            child: Stack(
                              ...
                              children: widget.children.asMap().map((index, child) => MapEntry(index, _applyTransformations(child, index))).values.toList(),
                            ),
                          ),
                        )
                      : Container(),
          ...
          Container(
            ...
            child: RawMaterialButton(
              ...
              child: Center(child: _scaleAnimation.value == 1.0 ? widget.fabCloseIcon : widget.fabOpenIcon),
            ),
          ),
        ],
      ),
  ...
  Widget _applyTransformations(Widget child, int index) {
    ...
    return Transform(
        transform: Matrix4.translationValues(
            (-(_ringDiameter! / 2) * cos(angle) + (_ringWidth! / 2 * cos(angle))) * _directionX, (-(_ringDiameter! / 2) * sin(angle) + (_ringWidth! / 2 * sin(angle))) * _directionY, 0.0),
        alignment: FractionalOffset.center,
        child: Material(
  ...
  void _calculateProps() {
    ...
    if (_colorAnimation == null || !kReleaseMode) {
      _colorCurve = CurvedAnimation(parent: _animationController, curve: Interval(0.0, 0.4, curve: widget.animationCurve));
